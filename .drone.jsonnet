...
local __customStepBuilder(name = null, config = {}) = {
  validate: function (pipelineConfig) [] +
    if name == null then
      'Custom step definition is missing a "name".' +
    if !std.objectHas(config, 'image') then
      'Custom step "' + name + '" is missing a container image.',
...
local __pipelineFactory = {
  ...
  createPipeline(configuration = {}): {
    ...
    local validationErrors = validateSteps(pipelineConfig, config.steps),
    ...
    steps:
      if std.length(errors) > 0 then
        createErrorPipeline(config, errors)
      else
